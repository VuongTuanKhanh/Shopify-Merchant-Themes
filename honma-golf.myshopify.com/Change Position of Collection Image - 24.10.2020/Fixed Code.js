...
usf_sectionSettings['stock-overlay-threshold'] = 1
try {
...
switch (usf_sectionSettings.coll_layout_per_row_int) {
    case 2:
        itemClass = "half"
        break;
    case 3:
...
<div v-if="view === 'grid'" class="usf-sr-product grid__item product-block column usf-skeleton" :class="[itemClass]">
...
<div class="usf-sr-container" :class="{'usf-no-facets': noFacets, 'usf-empty': !loader && !hasResults, 'usf-nosearch': !showSearchBox}">
    ...
    <div :class="(view === \'grid\' ? \'product-list cf grid--uniform\'  : \'list-view-items\') + \' usf-results usf-\' + view">
...
<div class="product-block column" :class="['hover-' + usf_sectionSettings.hover_effect, itemClass, {'on-sale' : hasDiscount, 'sold-out' : isSoldOut }]" >
        <div class="product-block__inner">
            <div class="image  " :class="{'image--hover-second': usf_sectionSettings.hover_effect == 'image' && product.images[1]}">
                <a class="image-inner" :href="productUrl" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave">
                    <div class="image__first">
                        <div class="rimage-outer-wrapper" :style="'max-width: ' + selectedImage.width + 'px'">
                            <div class="rimage-wrapper " >
                                <img class="rimage__image fade-in lazyautosizes lazyloaded usf-inherit" :data-widths="dataWidths" :data-aspectratio="dataAspectratio(selectedImage)" data-sizes="auto" alt="" :data-srcset="_usfGetImageUrls(scaledSelectedImageUrl)" sizes="339px" :srcset="_usfGetImageUrls(scaledSelectedImageUrl)" :id="'primary-img-' + product.id" @load="imageLoaded('primary-img-' + product.id)">
                                <noscript>
                                    <img :src="selectedImageUrl" alt="" class="rimage__image">
                                </noscript>
                            </div>
                        </div>
                    </div>
                    <div class="image__second" v-if="usf_sectionSettings.hover_effect == 'image' && hoverImageUrl">
                        <div class="rimage-outer-wrapper" style="max-width: 512px">
                            <div class="rimage-wrapper " >
                                <img class="rimage__image fade-in cover lazyautosizes lazyloaded usf-inherit" :data-widths="dataWidths" :data-aspectratio="dataAspectratio(hoverImage)" data-sizes="auto" alt="" data-parent-fit="cover" :data-srcset="_usfGetImageUrls(scaledHoverImageUrl)" sizes="457.3596837944663px" :srcset="_usfGetImageUrls(scaledHoverImageUrl)" :id="'second-img-' + product.id" @load="imageLoaded('second-img-' + product.id)">
                                <noscript>
                                    <img :src="hoverImageUrl" alt="" class="rimage__image">
                                </noscript>
                            </div>
                        </div>
                    </div>
                    ...
                    <!--sale label-->
                    <span class="product-label" :class="{'sale' : hasDiscount && usf.settings.search.showSale && usf_themeSettings.show_sale_label && !isSoldOut, 'unavailable' : isSoldOut && usf.settings.search.showSoldOut && usf_themeSettings.show_sold_out_label }">
                        <span  v-if="hasDiscount && usf.settings.search.showSale && usf_themeSettings.show_sale_label && !isSoldOut" v-html="loc.sale"></span>
                        <span v-if="isSoldOut && usf.settings.search.showSoldOut && usf_themeSettings.show_sold_out_label" v-html="loc.soldOut"></span>
                    </span>
                    <!-- product image extra -->
                    <usf-plugin name="searchResultsProductImageExtra" :data="pluginData"></usf-plugin>
                </a>
            </div>
            <div class="caption upper"  @mouseover="onItemHover" @mouseleave="onItemLeave">
                <!--title-->
            ...
            <div class="caption lower"  @mouseover="onItemHover" @mouseleave="onItemLeave">
                <a :href="productUrl" class="price">
                    <span v-if="hasDiscount">From</span>
                    <span class="theme-money" v-html="displayDiscountedPrice"></span>
...
<a class="usf-sr-product list-view-item" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave" :href="productUrl" :key="product.id">
    ...
    <div class="list-view-item__price-column">
        <span :href="productUrl" class="price">
            <span v-if="hasDiscount">From</span>
            <span class="theme-money" v-html="displayDiscountedPrice"></span>
            <span class="was-price theme-money" v-if="hasDiscount" v-html="displayPrice"></span>
        </span>
...
var imageLoaded = function (id) {
    var img = document.getElementById(id);
    if (img && img.parentElement.style.paddingTop == "") {
        img.parentElement.style.paddingTop = `${img.offsetHeight}px`;
        img.classList.remove('usf-inherit');
    }
}



var _usfImageWidths = [180, 220, 300, 360, 460, 540, 720, 900, 1080, 1296, 1512, 1728, 2048];
var dataWidths = _usfImageWidths.join(',');
...
var dataAspectratio = function (image) {
